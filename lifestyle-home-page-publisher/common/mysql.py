...
class Mysql(object):
    ...
    def __init__(self, host, port, user, pwd, db, charset):
        ...
        if self.__pool is None:
            self.__pool = PooledDB(creator=MySQLdb, mincached=1, maxcached=5, host=self._host, port=self._port,
                                   user=self._user, passwd=self._pwd, db=self._db, use_unicode=False,
                                   charset=self._charset, cursorclass=DictCursor)
