...
from blog.setting import BASE_PATH, FILE_MANAGER_PATH, STATIC_URL
...
class CkFinder(object):
    ...
    def dirlist(self, request):
        ...
        dir = request.arguments.get(u'dir', file_manager_path)[0]

        try:
            dir_path = root + dir
            if type(dir_path) is types.StringType:
                dir_path = dir_path.decode('utf8')
            for f_item in os.listdir(dir_path):
                f_sub = os.path.join(dir_path, f_item)

                if os.path.isdir(f_sub):
                    result.append(
                        '<li class="directory collapsed"><a href="#" rel="%s/">%s</a></li>' % (dir + f_item, f_item))
                else:
                    ext = os.path.splitext(f_item)[1][1:]
                    result.append(
                        '<li class="file ext_%s"><a href="#" rel="%s">%s</a></li>' % (ext, (dir + f_item), f_item))
    ...
    def get_info(self, request_path):
        ...
        if os.path.isdir(path):
            thefile = {
                ...
                "Preview": fileicons_path + '_Open.png',
                "Properties": {
                    "Date Created": '',
                    "Date Modified": '',
                    "Width": '',
